...
void genotype::init_means(bool is_missing) {
	...
	for(int i = 0; i < Nsnp; i++) {
		...
		if (is_missing) {
			columnmeans[i] = sum * 1.0 / (Nindv - static_cast<int>(not_O_i[i].size()));
		} else {
	...
	while (std::getline(ifs, line)) {
		...
		for(int j = 0; j < Nindv; j++) {
			...
			if (val == 0) {
				...
				std::cout << "Invalid entry in Genotype Matrix" << std::endl;
				exit(-1);
			}
		}
		i++;
	...
	while (std::getline(ifs, line)) {
		...
		for (int j = 0; j < Nindv; j++) {
			...
			if (val == 0 || val == 1 || val == 2) {
				...
				std::cout << "ERROR: Invalid character in genotype file" << std::endl;
				exit(-1);
			}
		}
		i++;
...
// Geneotype Data - Functions to read binary files
...
template <class T>
inline void printvector(std::vector<T> &t, std::string delim = " ", bool newline = false) {
...
template <class T>
inline void printvectornl(std::vector<T> &t, std::string delim = " ") {
...
void genotype::read_genotype_eff (std::string filename,bool allow_missing){
	...
	std::cout<<Nbits_hori<<"  "<<Nbits_ver<<"  "<<Nelements_hori<<"  "<<Nelements_ver<<std::endl;
    ...
    do{
		...
		if(ch=='\n'){
			...
			if(val==0){
				...
				std::cout<<"Invalid entry in Genotype Matrix"<<std::endl;
				std::cout<<"If there is Missing data, run with -miss flag"<<std::endl;
				exit(-1);
			}
			j++;
		}
